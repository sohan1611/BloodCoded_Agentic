...
## 2026-09-12/13 — Accuracy rescue, phases 1 and 2 (Claude, direct)
- Sohan reported four numeric symptoms on the live site. All four decoded to exact lines, and every reported value was a member of the small finite orbit the fixed parameters generate from a 0.5 prior in <= 5 observations.
- THE FLOOR. `transition()` added (1-p)*0.15 after EVERY observation including failures, so the wrong-answer map had an attracting fixed point at T(1-G)/(1-G-S) = 6/35 = 0.1714. Not a clamp -- which is why nobody found it by grepping for max(). Now expressed in closed form as `analytic_floor()`, which still reproduces 6/35 for the old parameters; that regression test is what makes the new 0.096 credible rather than another guess.
- THE JUMP. p_guess 0.20 is the 1-in-5 multiple-choice figure and nothing here is multiple choice. It is now derived per item from how falsifiable the graded suite is (1 distinct expectation -> 0.20, 2 -> 0.10, 3+ -> 0.05). This prices our own diagnostic honestly: every diagnostic question has ONE expected output and FOUR OF THE EIGHT expect the literal "10", so `print(10)` passes half of it.
- EVIDENCE QUALITY. `score` (test-case pass fraction) was computed at nodes.py:741 and discarded, so a typo and a wholly wrong program were numerically identical events. Now a tempered likelihood: odds *= L**w with the transition scaled by the same w. Weighting the transition too is load-bearing -- at full T a syntax error would have moved mastery UP from 0.300 to 0.307, rewarding a student whose program never ran.
- ATTRIBUTION. The system already diagnosed a NameError as `prerequisite_hint="variables"` and used it to pick the NEXT problem, while still taking the whole mark off loops. Failures are now split 0.40/0.60 with the implicated DIRECT prerequisite; shares partition one observation so double counting is impossible by construction. RULE 4 trap avoided: `implicates` is written by the LLM branch too, so a SEPARATE `attribution_hint` carries only the deterministic pattern's verdict, and only that touches a mastery score.
- CONFIDENCE. Was 1-e^(-n/4) on the raw attempt count: five right and five wrong both returned 0.7135 and it could never fall. Now agreement-based, anchored on the model's own poles (knows -> 1-slip, does not -> guess). 5 correct 0.811, 3 wrong 0.632 (confidently NOT mastered), alternating 0.231, never tested 0.000.
- LAMBDA WAS MEASURED, NOT CHOSEN. At 0.80 the demo e2e test failed: policy caps a skill at 4 attempts/session, so a student who fails twice then succeeds twice ends at confidence 0.265 and can NEVER be marked mastered however well they finish. 0.60 puts that recovery at 0.511 while a clean run stays 0.811 and alternating stays 0.231. The failing test was the signal; there is now a test named for that case.
- DEMO_SEED was fabricated and the new model exposed it: attempts=3 written next to confidence up to 0.85, a pair no formula could produce. Evidence counters are now derived from the claimed confidence so the seed is self-consistent.
- Codex was given work order 17 and exited twice without writing a byte (zero stdout, process gone). Killed and implemented directly, as on WO16. Delegation has now failed three times running on this machine.
- Gate: 517 passed, 5 skipped. tests/test_safety_invariant.py byte-identical and green (RULE 3). app/mastery imports nothing from graph/api/llm/agents/rag/services (RULE 4).
- Phase 3 (surface): the exercise card's "Based on: 04_loops.md#4.1 Why loops exist" printed our own filenames and anchors at students; replaced with what the exercise is testing and how it is graded, both already in the payload. The belief panel showed a bare mastery figure with no evidence beside it, and `confidence_scores` was never even shipped in the tutor view -- it is now, alongside a `measured` list, and an untested skill reads "not checked yet" instead of a number that looks measured. The heatmap drew 318px (sixteen 14px columns) however wide the card was; twenty-six 18px columns is 602px and half a year rather than a quarter. The hour strip now labels the hours that hold data, so a single late-evening bar is not left standing over an unlabelled gap.
- Gate: 517 passed, 5 skipped; npm run typecheck and npm run build both exit 0.
- Phase 4 (the invisible fault): no module under app/tools/sandbox held a logger, so a sandbox fault reached Render as a plain `POST /submit 200 OK` and nothing else -- the student was told something went wrong on our side and there was no way to find out what. `_sandbox_error` now logs `[sandbox-fault] execution never started: <ExcType>: <message>` at WARNING, chosen deliberately: the root logger is unconfigured under uvicorn, so logging.lastResort carries WARNING and above to stderr and an INFO line would have been dropped straight back into silence. The frontend also stopped discarding `feedback.message`, so a refusal now says which import was refused instead of apologising blankly.
- Documentation honesty: README said "fitting did not beat literature defaults, so we ship the defaults" -- a claim that became FALSE the moment three of four parameters changed, since that study ran against the old ones. Reworded to report the negative result exactly as measured, against the parameters in force at the time, and to state plainly that the new values are argued from the grading model rather than fitted and that the study does not validate them. docs/COSTS.md called the fit "EM on synthetic trajectories" while eval/bkt_fit.py:17-21 says explicitly it is NOT EM but coarse grid search plus local refinement -- a pre-existing false claim, now corrected. ARCHITECTURE.md and DEMO_SCRIPT.md swept for the same.

## 2026-09-13 — Work order 18 (Codex implemented, Claude reviewed) — "not checked yet" is a state
- FIRST THING TO RECORD, because it invalidates three earlier entries: Codex was never broken. Every launch had been wrapped in `cmd /c "codex exec ..."`, which on this machine opens a shell, prints the Windows banner and exits WITHOUT running the command -- so codex never started and produced zero bytes, which I misread three times as the tool failing. Called directly from bash (`codex exec -C <dir> -s workspace-write - < spec`) it answers in seconds. WO16 and WO17 were implemented by Claude for no reason. Test the invocation before blaming the tool.
- WO18 (Codex): skills.yaml stripped to the DAG with the prior moved to SkillNode defaults guarded by a test against BKTParams.p_init; the diagnostic's invented `min(inherited, 0.55)` for skipped skills deleted and replaced with `skipped_because`; /plan and /progress return null mastery/confidence for unmeasured skills plus an `unmeasured` state, `not_measured_because`, four-way partitioned counts and measured-only overall_mastery; web types `number | null` forcing every render site; scripts/reset_mastery.py plus a transactional StudentStore.reset_mastery.
- Review (Claude) ran everything Codex could not -- its sandbox cannot spawn processes (`CreateProcessAsUserW failed: 5`), so its "verification" was a syntax parse and nothing more. pytest, typecheck and build were all run here.
- REJECTED once: test_a_difficulty_change_arrives_with_its_reason failed. Diagnosis is worth keeping because it is NOT a defect in the change: the honest prior moved a new student from 0.5 to 0.30, `_difficulty_for` returns EASY below 0.40, and policy.py:269 deliberately answers repeated failure at EASY with EXPLAIN_DIFFERENTLY because no rung is lower. Real behaviour change, stale test premise.
- Codex's correction FAILED review too: it seeded mastery 0.99 to get above EASY, but is_mastered(0.99, 0.736) is True so the session completed before ever serving a problem to fail, and it left a stale `== "HARD"` assertion behind. Fixed here rather than spending a third round trip on submission day: 0.55 sits between two edges that both matter -- at or above 0.40 so a step DOWN exists, below MASTERY_THRESHOLD 0.60 so the skill is not already complete. Both edges are now stated in a comment so the next person does not rediscover them.
- Codex also added the regression the failure revealed and nothing guarded: a brand-new unmeasured student starts at EASY and repeated failure there produces EXPLAIN_DIFFERENTLY with difficulty unchanged.
- Gate: 524 passed, 5 skipped; typecheck and build clean; no `?? 0`, `!` or `as number` anywhere in the web diff (any of them would have put the original bug straight back); tests/test_safety_invariant.py byte-identical.

## 2026-09-13 — "Can't reach the tutoring engine": the cause was never the engine
- Sohan hit the waking banner repeatedly, then the offline state, and asked whether only one person can use the engine at a time. Chased it properly rather than guessing, and every hypothesis died on evidence:
  - ONE USER AT A TIME: false. Six concurrent /health requests all returned 200 in under 0.5 s.
  - EXTENSION BLOCKING: false. Incognito failed too.
  - CSP: false. No CSP header, no meta http-equiv, no securitypolicyviolation event fires.
  - CORS: false. Preflight from the live origin returns 200 with the right allow-origin, methods and headers.
  - WRONG URL: false. `https://cogniflow-engine.onrender.com` is correctly inlined in the shipped bundle.
  - A CLEAN BROWSER, loading the real deployed origin, fetched the engine cross-origin and got 200. The app code, the CORS config and the engine are all correct.
- WHAT IT ACTUALLY WAS: cold starts, plus two defects that turned "slow" into "broken".
  1. The wake window was 180 s, justified in its own comment as "about three times the measured cold start". That multiple was computed against a cold start we no longer have: uvicorn alone takes ~30 s from launch to "corpus chunks indexed", and door-to-door waits of 72 s AND 166 s were both measured on the same day. 180 s was barely 1.1x the worst case, so a healthy engine that booted slowly tipped the page into "Can't reach the tutoring engine" while it was still starting. Now 360 s.
  2. The banner promised "about a minute". A student watching 166 s was being told the app was late when it was behaving exactly as designed. Now states one to three minutes, and that nothing needs refreshing.
- A NOTE ON MEASUREMENT, because it nearly sent me the wrong way twice: an injected fetch from the Claude-in-Chrome extension fails in ~1 ms for ANY cross-origin URL. That is the automation sandbox, not the page. Same-origin fetches from the same injected context succeed, which is what makes it look like a real network failure. The page's own requests are the only trustworthy signal, and Render's request log is the only trustworthy verdict on whether they arrived -- they never did, because they never left the browser during a cold start.
- ROOT FIX, not a mitigation: .github/workflows/keep-engine-warm.yml pings /health every ten minutes, so the free tier never suspends and no visitor -- judge included -- meets a cold start at all. It authors nothing and is granted `permissions: {}`, so CLAUDE.md RULE 1 is untouched: RULE 1 forbids automation that COMMITS, and this makes one HTTP request and exits.
- Also applied by hand to production Postgres, since the app's migration is lazy and only runs on first connect: skill_mastery gained evidence_weight/agree_correct/agree_wrong, and attempt_log gained weight/attributed_from. Both verified against the live schema.
- WO20 (Codex): removed the outermost floating card only -- desktop `body` padding 26px -> 0, and all three `.shell` declarations lost `border-radius` and `box-shadow` and went to `padding: 0`, keeping `background: var(--shell)` (removing it would reintroduce the seam the branding work fixed) and `max-width: 1420px; margin: 0 auto` (the card edge was the target, not the measure). `.topbar` padding-left 0 puts the brand at the true corner; 18px kept on the right so the account chip is not flush. Verified by probe: `.shell` radius 0 / shadow none, while `.card` keeps 20px and `.panel` 22px -- Sohan's "only the outermost floating thing, not all floating" holds.
- REVIEW CATCH: `.auth-page` had `min-height: calc(100dvh - 52px)`, where 52 was exactly the 26px body padding this breakpoint used to add on each side. With the padding gone the sign-in screen sat 52px short of the fold -- measured gapAtBottom 52. Now `100dvh`, re-measured at gap 0 with no overflow. Worth remembering as a class of bug: a magic number in one rule that silently encodes another rule's value will not announce itself when that other rule changes.
- Follow-up: with the shell inset gone the brand sat at x=0 while the content sat at the plate's 32px, so they no longer lined up. Rather than hardcode 32 into the three topbar rules -- the same shape of mistake as the 52px above -- both now read a single `--app-gutter`, 16px on bare :root and 32px at >= 768px. Measured aligned at 400, 900 and 1600px.
- Refinement: flush was not what Sohan wanted after all -- the header should FRAME the page, brand hanging slightly left of the "Good morning / CONFIRMED" line and the account chip the same distance right. Added `--header-gutter`: equal to `--app-gutter` on a phone (16px, no room to spare) and half of it from 768px up, so the header hangs 16px outside the content column on both sides. Logo 44px -> 52px. Measured: 1600px content 32 / header 16; 390px both 16.
- NOT DONE, deliberately: Arka's account (29e09c3e, gamingarka07@gmail.com, 17 attempts) was NOT reset. Sohan asked to reset "my account", and the obvious guess -- the only account with history -- would have wiped his teammate's work. Looking up neon_auth."user" first showed Sohan's own account (15652478) has NO student row at all: no skills, no attempts. A student row is created on first SESSION, not at sign-up, and he had never got past the engine, so there was nothing to reset. Match on identity, never on "the row that looks right".

## 2026-09-13 — three reports from Sohan, and the logo one had a cause CSS could not reach
- LOGO STILL NOT ALIGNED. The `--header-gutter` change moved the image BOX 16px left of the content column, and the mark still looked right of it. Cause: each logo file is ~30% blank margin on every side (light: content x155..356 of 512; dark: x154..357). CSS can only move the box; the mark floats inside it, so at a 52px height the visible mark started ~31px in from its own left edge. Cropped both to their ink (202x184 and 204x186) so the box IS the mark and every alignment rule now means what it says. Re-encoded at quality 97 with no chroma subsampling; corner pixels checked against the page background afterwards -- dark exact, light 2/255, inside the variance the untouched originals already had. Intrinsic width/height updated in shell.tsx and auth-ui.tsx so nothing shifts on load, and the CSS height dropped 52 -> 44 because the same number now draws a bigger mark.
- FROZEN LOGIN AFTER GOOGLE + BACK BUTTON. Real bug, and a nasty one: `setPending("google")` disables the form, the browser leaves for Google, and pressing back restores the page from the bfcache with React state intact -- `pending` still "google" -- so every control stays disabled. A bfcache restore does not remount, does not re-run effects and fires no event a React app notices, which is why only a manual reload cleared it. `useClearOnRestore` listens for `pageshow` with `persisted` and clears the flag; applied to sign-in and sign-up, which had identical code.
- NO EMAIL VERIFICATION. Confirmed from the data rather than assumed: neon_auth."user"."emailVerified" is TRUE only for the Google account and FALSE for both email/password ones. So anyone can register any address they do not own. This is Neon's Managed Better Auth, not our code -- `update_auth_config` over MCP exposes only `name`, so there is no toggle available from here. Reported to Sohan as a hosting-console question with the honest hackathon framing.
- Also confirmed the earlier deletion worked and was not undone: system@gmail.com reappeared with a NEW id (b34facb8, created 05:23) after Sohan re-registered it to demonstrate exactly this gap.
- Header now carries the ICON ALONE at 44px, Sohan's call after seeing the stacked lockup render its wordmark ~9px tall. Cropped the first ink BAND of each source (the icon sits above a blank gap above the wordmark, so band detection finds it without hardcoding coordinates): 158x132 light, 158x135 dark. The word is not lost -- it is the page heading and the tab title.
- One thing worth recording because it was not obvious: saving these losslessly as PNG did NOT fix the corner colour. Drift of 7-8/255 around the icon's rounded corners survived, because it is JPEG noise in the SOURCE pixels rather than anything the encoder did. Near-background pixels are now snapped to the exact page colour (threshold 30 summed across channels -- tight enough to catch compression noise on a flat field and leave the mark's antialiased edges alone). Corner drift measured 0/255 on both afterwards, which is what actually makes the tile invisible.
- The full-lockup JPEGs are now unreferenced and removed; they regenerate from the two source images at the repo root in one script if the wordmark is ever wanted back.
- "Still a gap between the edges and the logo/name": measured rather than guessed, and the gutter was innocent. `.shell` carried `max-width: 1420px; margin: 0 auto`, so on a 1920 screen the WHOLE app -- header included -- was centred with 242px of empty margin each side, and the 16px gutter was measured from there. The cap has moved off the shell and onto `.plate`, so the header now spans the window (logo 16px from the left edge, account chip 16px from the right; a reading of 31 on the right is the scrollbar, since innerWidth counts it and clientWidth does not) while the reading column stays capped at 1420 and centred. No horizontal overflow.
- TRADE-OFF TO REVISIT: the header is now at the extremes and therefore NO LONGER aligned with the content column (logo 16, content 274 at 1920px). Those two wishes are mutually exclusive while the content is capped for readability. Flagged to Sohan rather than silently picking one.

## 2026-09-13 — Work order 21 (Claude → Codex): a variable mistake in a loop that does not crash
- Evidence, not hypothesis: attempt_log 06:56:22 pop@gmail.com `loops WRONG_ANSWER 0.674->0.337 weight 1.000 attributed_from NULL`. Sohan made a variable mistake in a loop; loops took the whole debit.
- Root cause: of seven PATTERNS exactly one hinted `variables` -- `name_error`, which matches only a NameError in stderr. A variable mistake that runs and prints the wrong answer produces no stderr, so attribution never had anything to fire on. Wiring correct; rule library missing the case.
- Two design constraints put in the spec, both found by reading before delegating: (1) `analyse_draft`/`DraftFacts` already parse with ast but are documented as hint wording ONLY, never routing or mastery -- and detect() now drives mastery -- so a separate analysis of SUBMITTED code was required rather than reuse. (2) THE TRAP: detect()'s second loop returns any pattern that has `outcomes` but no `code_patterns` for EVERY submission of that outcome (how infinite_loop claims all timeouts). A naive "variables on WRONG_ANSWER" rule would have sent 60% of every loops debit to variables -- the reported inaccuracy, inverted.
- Codex: optional `code_check` field on Pattern; two fail-closed ast predicates (`resets_accumulator_inside_loop`, `overwrites_instead_of_accumulating`) with an OWN-BODY rule that stops at nested loops, so the correct per-row idiom (reset in the outer loop, accumulate in the inner) cannot match; patterns `accumulator_reset_in_loop` and `overwrite_instead_of_accumulate`, WRONG_ANSWER only, placed after print_instead_of_return. 30 tests added including the negative cases. Could not run pytest (sandbox cannot spawn processes, as always).
- Review (Claude): 558 passed, 5 skipped (+30); zero lines removed from existing tests; safety invariant byte-identical; no forbidden imports; no placeholders; trap confirmed closed both by reading (code_check hits `continue` before the match-everything branch) and empirically (`print(1)` WRONG_ANSWER still None). End-to-end through the real modules: reset or overwrite on a loops exercise splits 0.40/0.60 and variables moves 0.674->0.442; the same mistake on nested_loops correctly refuses the split (variables is two hops); a CORRECT accumulator that is wrong for another reason gets no variables blame. ACCEPTED first round, no correction needed.
- Codex volunteered two definition-level misfires instead of quietly changing the definitions; both judged acceptable and recorded: (a) the reset rule is structural, so a reset and update in mutually exclusive branches still match -- rare, and resetting an accumulator inside a loop under any condition is still a state-lifetime question; (b) the overwrite rule can blame variables for intentional last-item tracking seeded with 0 or "" -- but it only fires on a WRONG ANSWER, where overwriting is usually the actual bug; the false positive needs deliberate last-item tracking AND an unrelated bug together.
- NOT changed, deliberately: Sohan's "intentional conditional mistake" (`x = 10; if x < 5: x = x + 1` beside a correct triangle) was credited to nested_loops alone. A false condition is valid code, the program printed the right answer, and a correct submission carries no evidence of any misconception. Penalising it would be manufacturing a false positive.
- Process note: five codex processes were still running afterwards. Checked command lines before killing anything -- all five were Sohan's Codex DESKTOP APP (WindowsApps OpenAI.CodexBeta, its app-server and sandbox), not this delegation, which had exited. Killing by image name would have closed his session. Identify before you terminate.
